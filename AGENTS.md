...
2. **Avatar**: a Live2D (Cubism 4) model — the free Mao sample — rendered in the corner overlay via pixi.js. Mouth flap driven by the TTS talking signal; cursor gaze + mood expressions layered on top. This is the sole avatar; the old PNG sprite stage was retired.
...
The avatar is the **Live2D Mao model** at `apps/stage-xena/assets/live2d/mao/` (Mao.model3.json + textures, motions, expressions). It is the one true model — other Live2D models were removed by user directive. Visuals are tuned via `Mao.moc3` parameters and the expression/motion maps in `apps/stage-xena/src/renderer/modules/avatar/live2d/stage.ts`.

When the user supplies a different/upgraded Live2D model it replaces Mao one-to-one under `assets/live2d/<name>/`.
...
- Provider failover (2026-08-26, ADR-001): chat + proactive walk 9Router (`oc/big-pickle`) → free `oc/*` fallbacks (`oc/laguna-s-2.1-free`, `oc/mimo-v2.5-free`); vision chain 9Router (`oc/x-preview-f-free`) → `oc/mimo-v2.5-free` → `oc/laguna-s-2.1-free`. All targets on the 9Router gateway — no paid provider keys. Streams never restart after first token. Config in `.env` (`XENA_FALLBACK_TEXT_MODELS`, `XENA_FALLBACK_VISION_MODELS`).
- Memory recall (2026-08-26): keyword+recency scoring over past transcripts (`xena-core/memory/recall.ts`), top fragments injected into system prompt; no embeddings, zero extra inference.
- Emotion system (2026-08-26): model leads replies with one mood tag (`[happy] [smug] [surprised] [annoyed] [sleepy]`); parsed in main, stripped from speech/UI, drives Live2D face (Mao expression ALTERNATES + TapBody motions) with 12s mood decay. E2E-verified via CDP.
- Liveliness (2026-08-26): procedural idle breathing + talk bounce — lived on the PNG sprite stage, retired with it.
- Live2D avatar (2026-08-26, tray toggle): pixi.js 6 + pixi-live2d-display (cubism4) rendering the free Mao sample model in the corner overlay (Mao is the ONLY model; others removed). Mouth flap via ParamMouthOpenY; moods pick RANDOM expression ALTERNATES (happy sparkles/content, surprised wide/blush) + fire TapBody motions (6 gestures) for all expressive moods. Requires `@pixi/unsafe-eval` (CSP keeps script-src 'self') + pnpm overrides pinning @pixi/* 6.5.10 (duplicate-instance bug). Verified on-screen via CDP screenshots.
- PNGTuber retired (2026-08-27): Live2D is now the sole avatar. The PNG sprite stage (EmoteStage/MouthFlap/Liveliness + idle/talk/blink + emotion sprites) and its generator script were removed; `Avatar` tray toggle controls visibility (default ON).
- Polish wave (2026-08-26): reasoning "thinking" indicator (orange dot + ellipsis while reasoning_content streams), bar input history (ArrowUp/Down, last 20), Start-with-Windows toggle, TTS voice picker (7 Edge voices incl. JA, tray radio), one-time first-run greeting, failover circuit breaker (5min fallback cooldown after fallback quota failure), `/forget <keyword>` + `/help` commands, ambient screen glances (opt-in default OFF, 30min, visible one-line vision observations, quiet-hours gated), L2D ticker capped 30fps.
- Guided tasks (2026-08-27): natural-language instructional requests start a multi-step desktop tutor. Each step captures the full screen, vision chooses next visible action plus normalized coordinates and human-readable instruction, PointerWindow glides there, then waits for visual screen change before recapturing. Model decides done, clarification, or unknown. Pointer spins out on completion; direct `/point` and `[point:]` protocol removed.
- Transcripts moved to SQLite (2026-08-26): node:sqlite DatabaseSync (Node 24 in Electron 44, zero native deps), data/transcripts.db; legacy JSON imported id-deduped. Diary + facts remain human-editable files.
- UX pass (2026-08-26, user-requested): chatbox bottom-left translucent (panel 0.72), message queue while streaming (max 2, auto-send), markdown-lite answers (escape-first bold/italic/code), idle mood flickers every 5-12min, failure toast when all providers dead (5min throttle), TTS 96kbit (endpoint max), Mao L2D model + decoded per-model expression maps.
- Mao focus (2026-08-26, user directive): Mao is the ONLY Live2D model (others removed). Expression ALTERNATES per mood (random pick), TapBody motions (6 gestures) on all expressive moods, neutral reset (exp_01) on mood decay, jittered lip-sync. GAZE: GazeTracker polls cursor 120ms -> eyes+head follow (applied post-motion via internalModel.update patch); guided-task pointer targets can drive her gaze. Auto-fact memory: model appends [fact: ...] tags -> FactsStore. Welcome-back greeting on corner summon. Pointer glide + task completion exit. Voice: sole JP Nanami (ja-JP-NanamiNeural), always the happy read regardless of face mood — voice picker removed.
- Voice input (2026-08-26, UNBLOCKED): 9Router gained free `tokenrouter/openai/gpt-audio-mini` — probe verified TTS speech -> transcription -> answer. Push-to-talk Ctrl+Alt+V: renderer mic 16k mono -> WAV b64 -> transcribe (gpt-audio-mini -> gpt-audio, wrapper-stripped) -> auto-submit as chat. Mic permission handler; tray toggle; bar listening/transcribing states.
- Next: real Live2D art assets from user (replaces Mao one-to-one). Roadmap v0.1→v2.0 all shipped.
