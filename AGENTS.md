...
- Provider failover (2026-08-26, ADR-001): chat + proactive walk 9Router → OpenRouter (`stealth/ox-alpha`) on quota/auth/5xx/empty-200; vision chain 9Router (`oc/x-preview-f-free`) → `minimax/minimax-m3:free` → `stealth/ox-alpha`. Streams never restart after first token. Config in `.env` (`OPENROUTER_*`, `XENA_FALLBACK_*`).
...
- Emotion system (2026-08-26): model leads replies with one mood tag (`[happy] [smug] [surprised] [annoyed] [sleepy]`); parsed in main, stripped from speech/UI, drives avatar face (10 generated emotion sprites) + Live2D motions. E2E-verified via CDP.
- Liveliness (2026-08-26): procedural idle breathing + talk bounce (compositor-only CSS transforms).
- Live2D stage (2026-08-26, experimental, tray toggle, default OFF): pixi.js 6 + pixi-live2d-display (cubism4) rendering the free Mao sample model in the corner overlay (user directive: Mao is the ONLY model; others removed). Mouth flap via ParamMouthOpenY; moods pick RANDOM expression ALTERNATES (happy sparkles/content, surprised wide/blush) + fire TapBody motions (6 gestures) for all expressive moods. Requires `@pixi/unsafe-eval` (CSP keeps script-src 'self') + pnpm overrides pinning @pixi/* 6.5.10 (duplicate-instance bug). Footprint: ~229MB PNG mode, ~322MB with L2D. Verified on-screen via CDP screenshots.
...
- AI Pointer (2026-08-26, user-requested): dual-cursor system. `/point <thing>` or natural-language `[point: target]` tag (persona appends it when directing user to visible UI; "plausibly visible" guard) -> screen capture -> vision model returns normalized JSON coords -> PointerWindow (transparent click-through overlay, pulsing ring + label chip, 9s dwell) appears at full-display-bounds-mapped position. Verified visually on Firefox taskbar icon.
...
- Mao focus (2026-08-26, user directive): Mao is the ONLY Live2D model (others removed). Expression ALTERNATES per mood (random pick), TapBody motions (6 gestures) on all expressive moods, neutral reset (exp_01) on mood decay, jittered lip-sync. GAZE: GazeTracker polls cursor 120ms -> eyes+head follow (applied post-motion via internalModel.update patch); lookAtPoint override makes her glance at AI Pointer targets. Auto-fact memory: model appends [fact: ...] tags -> FactsStore. Welcome-back greeting on corner summon after 30min absence. Multi-step [point:] choreography (sequential, 4.5s apart). Pointer glide + locate retry. TTS per-mood prosody (SSML rate/pitch/volume).
